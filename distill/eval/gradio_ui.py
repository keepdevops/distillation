...
python -m distill.data.expert --mode inspect --dataset my_data.jsonl

# 2. Remap columns to instruction/output
python -m distill.data.expert --mode remap \\
  --dataset my_data.jsonl --instruction_col question --output_col answer

# 3. Generate Chain-of-Thought via GGUF teacher
python -m distill.data.expert --mode cot --domain tax

# 4. Run full distillation on CoT data
python -m distill.data.expert --mode distill --domain tax
```
...
```bash
python -m distill.eval.quality ./distilled-mlx \\
...
```bash
python -m distill.eval.perplexity ./distilled-minillm \\
...
```bash
python -m distill.eval.benchmarks ./distilled-mlx \\
...
```bash
python -m distill.training_watchdog ./distilled-mlx \\
...
```bash
python -m distill.export.coreml \\
...
python -m distill.cache_models      # → hf_cache/
python -m distill.cache_datasets    # → datasets_cache/

# Or all-in-one
python -m distill.setup_airgap
